...
class DbRealm {
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = (principal instanceof String) ? User.findByUsername(principal): User.findById(principal)
        ...
        def results = (principal instanceof String) ? User.findByUsername(principal)?.roles : User.findById(principal)?.roles

        // There may be some duplicate entries in the results, but
        // at this stage it is not worth trying to remove them. Now,
        // create a real permission from each result and check it
        // against the required one.
        retval = results.find { role ->
            // Create a real permission instance from the database
            // permission.
            role.permissions.find { permString ->
                def perm = shiroPermissionResolver.resolvePermission(permString)
